...
def calculate_strategy_signals(df, current_res_minutes=60):
    """
    Рассчитывает сигналы и фичи для LSTM.
    """
    ...
    # Режим рынка
    df['regime'] = 'ranging'
    df.loc[(df['trend_score'] >= 3) & (df['volatility'] > 0.003), 'regime'] = 'trending_up'
    df.loc[(df['trend_score'] <= 1) & (df['volatility'] > 0.003), 'regime'] = 'trending_down'

    return df

def get_market_regime(df):
    """Возвращает режим последнего бара."""
    return df['regime'].iloc[-1] if not df.empty else 'ranging'
