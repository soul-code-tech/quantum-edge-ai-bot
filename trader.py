...
class BingXTrader:
   ...
   def place_order(self, side, amount, stop_loss_price, take_profit_price):
    try:
        ...
        # 🔍 Ждём и проверяем, какая позиция реально открылась
        ...
        # ✅ ИСПОЛЬЗУЕМ РЕАЛЬНЫЙ РАЗМЕР ПОЗИЦИИ — а не фиксированный amount!
        actual_amount = float(open_position['contracts'])
        print(f"📊 Используем реальный размер позиции: {actual_amount} BTC")

        # ✅ Стоп-лосс — на весь объём позиции
        print(f"⛔ Отправка стоп-лосса (stop_market) на {actual_amount} BTC: {stop_loss_price}")
        self.exchange.create_order(
            symbol=self.symbol,
            type='stop_market',
            side='sell' if side == 'buy' else 'buy',
            amount=actual_amount,  # ← ВАЖНО: используем actual_amount!
            params={'stopPrice': stop_loss_price}
        )

        # ✅ Тейк-профит — на весь объём позиции
